...
def local_max_baseline_with_x_ranges(x, y, x_ranges, min_x_distance=100):
    """
...
def main():
    ...
    after_collagen = r'../res/Agcube/04292025_0.1mgml_colgel_ployonsubstrate/HMR_3B_1/subspectrum/pixel_4_(211,254).mat'
    save_path = '../res/Agcube/04292025_0.1mgml_colgel_ployonsubstrate/HMR_3B_1/correction'
    os.makedirs(save_path, exist_ok=True)

    wavelengths = np.linspace(950, 1800, 426)

    data_after = loadmat(after_collagen)
    spectra = np.reshape(data_after['spectrum'], 426)
    spectra = savgol_filter(spectra, window_length=13, polyorder=2)
    upper_baseline, x_peaks = local_max_baseline_with_x_ranges(wavelengths, spectra, [(1000,1100),(1100,1200),(1200,1300),(1300,1400),(1700,1800)])
    flipped_spectrum_after = upper_baseline - spectra


    plt.figure(figsize=(20, 14))
    plt.plot(wavelengths, spectra, label='Raw Spectrum', color='r')
    plt.plot(wavelengths, upper_baseline, label='Baseline - After', color='purple')
    plt.plot(wavelengths, flipped_spectrum_after, label='Recovered Spectrum', color='b')
    plt.xlabel('Wavelength (cm⁻¹)')
    plt.ylabel('Intensity')
    plt.legend()
    plt.title('Local Maxi Baseline Correction')
    # plt.show()
    st()
    plt.savefig(os.path.join(save_path, 'Local_Maxi_Correction_Spectrum_pixel_4_(211,254).png'))

    output_filename = 'pixel_4_(211,254)_recovered_spectrum.mat'
