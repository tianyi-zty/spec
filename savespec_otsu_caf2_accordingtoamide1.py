...
def main():
    ...
    for foldername in foldername_list:
        for filename in filename_list:
            ...
            thresholds = threshold_multiotsu(region_after[:, :, 67].T, classes=3)
            # print(thresholds) 
            # thresholds = np.array([0.05, 0.1, 0.8])
            regions = np.digitize(region_after[:, :, 67].T, bins=thresholds)
            # st()
            ...
            data = binary_mask_region0.T[:, :, np.newaxis]*region_after
            ...
            print(f"Found {len(valid_spectra)} spectra with Amide I max between 1 and inf in {foldername}/{filename}")
            ...
            # Save selected spectra
            for count, spectrum in enumerate(selected_spectra):
                # second_derivative, minima_x, minima_y = process_spectrum(spectrum, wavelengths, 950, 1800, window=13, polyorder=2, prominence=0.0002)
