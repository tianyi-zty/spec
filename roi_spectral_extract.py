...
# in this code, i want to first visualize the spero data as an image at 1600cm-1[330;170;120;70] (change according to each resonance), then, generate a mask according to the dark pixel
...
def main():
    ...
    save_path = f'../res/after_cleaning/peptide1/AuPillars_50nmAl2O3_6_05232025/{filename}'
    ...
    x_start_1, x_end_1 = 306,470
    y_start_1, y_end_1 = 301,424
    region_before = spectra_before[x_start_1:x_end_1, y_start_1:y_end_1, :]
    x_start_2, x_end_2 = 250,414
    y_start_2, y_end_2 = 313,471
    region_after = spectra_after[x_start_2:x_end_2, y_start_2:y_end_2, :]

    n=60
    fig, ((ax1, ax2, ax3), (ax4, ax5, ax6)) = plt.subplots(2, 3, figsize=(12, 8))  # Create a 2x2 grid of subplots
    ...
    savemat(os.path.join(save_path, subspectrum_filename), {'spectrum': mean_spectrum_before})
    ...
    subspectrum_filename_2 = f"resonance_{filename}_spectrum_apply_mask.mat"
    ...
    ax1.plot(wavelengths, mean_spectrum_before, label='Before_Spectrum', color='g')
    # ax1.set_title('Full_region_Spectrum')
    ax1.set_xlabel('Wavenumber (cm⁻¹)')
    ax1.set_ylabel('Intensity')
    ax1.legend(loc='lower right')
    # Plot 1: Full region spectrum
    ax2.plot(wavelengths, mean_spectrum_after, label='Full_region_Spectrum', color='r')
    # ax1.set_title('Full_region_Spectrum')
    # ax2.set_xlabel('Wavenumber (cm⁻¹)')
    # ax2.set_ylabel('Intensity')
    ax2.legend(loc='lower right')
    # Plot 2: Mask1 region spectrum
    ax3.plot(wavelengths, mean_spectrum_after_roi, label='Mask1_region_Spectrum', color='b')
    # ax2.set_title('Mask1_region_Spectrum')
    # ax3.set_xlabel('Wavenumber (cm⁻¹)')
    # ax3.set_ylabel('Intensity')
    ax3.legend(loc='lower right')
    # Plot 3: Mask0 region spectrum
    ax4.plot(wavelengths, np.mean((1-binary_mask).T[:, :, np.newaxis]*region_after, axis=(0, 1)), label='Mask0_region_Spectrum', color='k')
    # ax3.set_title('Mask0_region_Spectrum')
